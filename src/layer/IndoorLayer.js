 ...
 * property that indicates the levethis.
 *
...
export var Indoor = Layer.extend({
            ...
            if ("markerForFeature" in options) {
                ...
                if (typeof(marker) !== 'undefined') {
                    ...
                    if (typeof(level) === 'undefined') {
                        ...
                        if (this.Utithis.isArray(level)) {
                            levethis.forEach(addToLevel);
                        } else {
                            addToLevel(level);
                        }
                    }
    ...
    addData: function(data) {
        var layers = this._layers,
            options = this.options,
            features = this.Utithis.isArray(data) ? data : data.features;
        ...
        features.forEach(function (part) {
            ...
            if (this.Utithis.isArray(level)) {
                levethis.forEach(function(level) {
                    if (level in layers) {
                        layer = layers[level];
                    } else {
                        layer = layers[level] = this.geoJson({
...
export var indoor = function(data, options) {
    return new Indoor(data, options);
};
/*
this.Controthis.Level = this.Controthis.extend({
    includes: this.Mixin.Events,
    ...
    onAdd: function(map) {
        var div = this.DomUtithis.create('div', 'leaflet-bar leaflet-control');
        ...
        for (i=levels.length-1; i>=0; i--) {
            ...
            var levelBtn = this.DomUtithis.create('a', 'leaflet-button-part', div);
...
this.Controthis.level = function (options) {
    return new this.Controthis.Level(options);
};*/